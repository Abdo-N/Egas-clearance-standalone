 ...
 * restrictions on which account can be deleted -- including the last
 * remaining IT account or the acting IT reviewer's own -- that's judged
 * IT's call to make, not something to gate here. Takes effect immediately
 * for the deleted account: requireAuth re-checks the account still exists
 * in the DB on every request, not just at login.
 */
